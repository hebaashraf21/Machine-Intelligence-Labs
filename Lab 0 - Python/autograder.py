...
class Problem:
    def __init__(self, **kwargs) -> None:
        ...
        if "function" in kwargs:
            self.default_fn = eval(kwargs["function"])
        self.default_cmp = default_comparator
        if "comparator" in kwargs:
            self.default_cmp = eval(kwargs["comparator"])
    ...
    def run(self, is_debug: bool = False, pattern: str = "*", time_scale: float = 1):
        ...
        test_cases = get_test_cases(os.path.join(
            root, self.testcases_path), pattern)
        self.grade = 0
        self.maximum_grade = 0
        for test_index, test_case in enumerate(test_cases):
            description = test_case.get(
                "description", f"Test Case {test_index+1}")
            timeout = test_case.get("timeout", self.default_timeout)
            if is_debug:
                print(
                    f"{test_index+1}: {description} :: time-limit is turned off in debug mode")
            else:
                print(
                    f"{test_index+1}: {description} :: time-limit = {timeout*time_scale} sec")
            fn = self.default_fn
            if "function" in test_case:
                fn = eval(test_case["function"])
            ...
            maximum_grade = self.weight * weight * \
                test_case.get("maximum_grade", 1)
            self.maximum_grade += maximum_grade
            result = run_test(fn, fn_args, cmp, cmp_args,
                              (None if is_debug else timeout * time_scale))
            if result is None:
            ...
            if result.success:
                ...
                print(
                    f"Result: FAIL {grade}/{maximum_grade} - {result.message}")
                if input_args:
...
def main(args: argparse.Namespace):
    ...
    if args.question != "all":
        try:
            ...
            if exclude:
                problems = [(problem, "*") for index,
                            problem in enumerate(problems) if index not in selected]
            else:
                problems = [(problem, selected[index]) for index,
                            problem in enumerate(problems) if index in selected]
        except:
            pass
    else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Automatically grades the solutions for the problem set")
    parser.add_argument("--question", "-q", default="all",
                        help="choose the question(s) to include in the grading (or prefix with ~ to exclude). A question number can be followed by a slash / followed by a glob pattern to filter the testcases.")
    parser.add_argument("--debug", "-d", action="store_true",
                        help="Disables timeout to enable debugging via the autograder")
    parser.add_argument("--timescale", "-t", type=str,
                        default="default", help="A scaling factor for the timeout")
