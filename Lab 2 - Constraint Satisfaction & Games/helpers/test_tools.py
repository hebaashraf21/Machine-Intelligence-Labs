...
def compare_csp_solve(
    ...
    message = f"Puzzle:{nl}{problem.format_assignment({})}{nl}Expected:{nl}{expected}{nl}Got:{nl}- Result: {format_solution(solution)}{nl}- Explored {explored} nodes"
    return Result(False, 0, message)

####################################################################
## Backtracking CSP Solve for CryptArithmetic Runner and Comparator

def run_csp_solve_cryptarithmetic(
    problem_cls_path: str,
    problem_file: str
    ...
    return solution, problem_file

def compare_cryptarithmetic_solution(
    output: Tuple[Optional[Assignment], str]) -> Result:

    output, problem_file = output

    with open(problem_file, 'r') as f:
        problem = f.read()
    pattern = r"\s*([a-zA-Z]+)\s*\+\s*([a-zA-Z]+)\s*=\s*([a-zA-Z]+)\s*"
    match = re.match(pattern, problem)
    if not match: raise Exception("Failed to parse:" + problem)
    terms = [match.group(i+1).upper() for i in range(3)]

    postfix = "\nPuzzle: " + problem

    # Check: There must be a solution
    if output is None:
        return Result(False, 0, f"Expected a solution but no solution was returned" + postfix)

    letters = list(set(''.join(terms)))
    
    # Check: All letters must be assigned a value
    missing_letters = [letter for letter in letters if letter not in output]
    if missing_letters:
        message = "The assignment does not contain values for the letters:"
        message += ', '.join(missing_letters)
        return Result(False, 0, message + postfix)

    # Check: The assigned value must be a digit
    digits = set(range(10))
    has_invalid_values = [letter for letter in letters if output[letter] not in digits]
    if has_invalid_values:
        message = "Some letters are assigned invalid values:\n"
        message += "\n".join(f"  - {letter}: {output[letter]}" for letter in has_invalid_values)
        return Result(False, 0, message + postfix)

    # Check: The first letter of each term must be >0
    firsts = [term[0] for term in terms]
    has_invalid_values = [letter for letter in firsts if output[letter] == 0]
    if has_invalid_values:
        message = "These letters cannot be assigned zero: "
        message += ", ".join(has_invalid_values)
        return Result(False, 0, message + postfix)

    # Check: No pair of letters must have the same value
    same = [(l1, l2) for i, l1 in enumerate(letters) for l2 in letters[i+1:] if output[l1] == output[l2]]
    if same:
        message = "The digit assigned to each letter must be unique, but your solution has:\n"
        message += "\n".join(f"  - {l1}={l2}={output[l2]}" for l1, l2 in same)
        return Result(False, 0, message + postfix)

    # Check that terms[0] + terms[1] = terms[2]
    terms = [int(''.join(str(output[l]) for l in term)) for term in terms]
    if terms[0] + terms[1] != terms[2]:
        message = f"Your solution {terms[0]} + {terms[1]} = {terms[2]} does not satisfy the equation"
        return Result(False, 0, message + postfix)

    return Result(True, 1, f"Your solution for the puzzle '{problem}' is {terms[0]} + {terms[1]} = {terms[2]}")
