...
def run_policy_iteration(env: GridEnv, discount_factor: float, iterations: int, noise: float = None) -> Tuple[UtilityMap, Policy, int]:
    ...
    # Reset the environment since we need to do that before requesting actions from the agent
    env.reset()
    # print("Actual Utility:", {tuple(s): u for s, u in extract_utilities(env, agent).items()})
    # actual_policy = agent.grading_policy()
    # m = {Direction.UP: 'U', Direction.DOWN: 'D', Direction.LEFT:'L', Direction.RIGHT:'R'}
    # p = {tuple(s):''.join(m[a] for a in ass) for s, ass in actual_policy.items()}
    # print("Actual Policy:", p)
    return extract_utilities(env, agent), extract_policy(env, agent), iterations
...
def compare_utility_policy_results(
    ...
    if type_mismatch is None:
        ...
        iteration_match = (iteration_test is None) or (
            eval("lambda value: " + iteration_test)(iterations))
    ...
    if type_mismatch is None:
        utility_side_to_side = side_by_side(format_utilities(
            expected_utility, mdp.size), format_utilities(utility, mdp.size))
        message += f"Utility: Expected vs Your Anwser{nl}{utility_side_to_side}{nl}{nl}"
        policy_side_to_side = side_by_side(format_policy_patterns(
            expected_policy, mdp.size), format_policy(policy, mdp.size))
        message += f"Policy: Expected vs Your Answer{nl}{policy_side_to_side}{nl}{nl}"
        if not utilities_match:
            message += "DIAGNOSIS: Utilities do not match the expected output\n"
        if not policy_match:
            message += "DIAGNOSIS: Policy do not match the expected output\n"
        if not iteration_match:
            message += f'DIAGNOSIS: The number of iterations done by the agent (value={iterations}) does no satisfy the condition "{iteration_test}"' + "\n"
    else:
...
def compare_q_policy_results(
    ...
    nl = '\n'
    message = f"Grid:{nl}{mdp}{nl}"
    q_values_side_to_side = side_by_side(format_q_values(
        expected_q_values, mdp.size), format_q_values(q_values, mdp.size))
    message += f"Q-Values: Expected vs Your Anwser{nl}{q_values_side_to_side}{nl}{nl}"
    policy_side_to_side = side_by_side(format_policy_patterns(
        expected_policy, mdp.size), format_policy(policy, mdp.size))
    message += f"Policy: Expected vs Your Answer{nl}{policy_side_to_side}{nl}{nl}"
    if not q_values_match:
        message += "DIAGNOSIS: Q-Values do not match the expected output\n"
    if not policy_match:
        message += "DIAGNOSIS: Policy do not match the expected output\n"
...
def compare_weights_policy_results(
    ...
    if not weights_match:
        message += "DIAGNOSIS: Weights do not match the expected output\n"
    if not policy_match:
        message += "DIAGNOSIS: Policy do not match the expected output\n"
...
#############################################
## SNAKE TEST CASE: RUNNERS & COMPARATORS #########
#############################################

# a runner to test a snake environment
# it takes as input a snake environment and a list of actions, trajectory
# trajectory is a list of actions that the snake agent will take in the environment
def run_snake_env(
        env: Environment[S, A],
        trajectory: List[A]) -> Tuple[S, List[float], bool, Optional[List[A]]]:
    """
    ...
    """
    state = env.reset(123)  # states
    rewards = []  # rewards
    done = False  # whether the game is done or not
    ...
    actions = None if done else env.actions()

    return state, rewards, done, actions


def compare_snake_env(
    output: Tuple[S, List[float], bool, Optional[List[A]]],
    expected_state: Tuple[Tuple[Point], Direction, Point],
    expected_rewards: List[float],
    expected_done: bool,
    expected_actions: Optional[List[A]]
) -> Result:
    """
    ...
    state, rewards, done, actions = output
    
    if expected_state is not None:
        expected_snake, expected_direction, expected_apple = expected_state
        if state.snake != expected_snake:
            message = "In the last state, the snake is incorrect.\n"
            message += "Expected: " + str(expected_snake) + "\n"
            message += "Got: " + str(state.snake) + "\n"
            return Result(False, 0, message)
        elif state.direction != expected_direction:
            message = "In the last state, the direction is incorrect.\n"
            message += "Expected: " + str(expected_direction) + "\n"
            message += "Got: " + str(state.direction) + "\n"
            return Result(False, 0, message)
        elif state.apple != expected_apple:
            message = "In the last state, the apple is incorrect.\n"
            message += "Expected: " + str(expected_apple) + "\n"
            message += "Got: " + str(state.apple) + "\n"
            return Result(False, 0, message)
    
    if rewards != expected_rewards:
        message = "The collected rewards are incorrect.\n"
        message += "Expected: " + str(expected_rewards) + "\n"
        message += "Got: " + str(rewards) + "\n"
        return Result(False, 0, message)
    elif done != expected_done:
        message = "The done flag after the last action is incorrect.\n"
        message += "Expected: " + str(expected_done) + "\n"
        message += "Got: " + str(done) + "\n"
        return Result(False, 0, message)
    elif expected_actions is not None and set(actions) != set(expected_actions):
        message = "The possible actions for the last state are incorrect.\n"
        message += "Expected: " + str(expected_actions) + "\n"
        message += "Got: " + str(actions) + "\n"
        return Result(False, 0, message)
