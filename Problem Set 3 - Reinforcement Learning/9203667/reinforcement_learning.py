...
class RLAgent(Agent[S, A]):
    ...
    def act(self, env: Environment[S, A], observation: S, training: bool = False) -> A:
        ...
        if training and self.should_explore():
            ...
            random_index = self.rng.int(0, len(actions) - 1)
            ...
            max_q_action = max(actions, key=lambda action: self.compute_q(env, observation, action))
            return max_q_action
...
class QLearningAgent(RLAgent[S, A]):
    ...
    def compute_utility(self, env: Environment[S, A], state: S) -> float:
        ...
        # for all actions, check if the action has a q value larger than the max and update the current max value
        max_q = max(self.compute_q(env, state, action) for action in env.actions())
...
class ApproximateQLearningAgent(RLAgent[S, A]):
    ...
    # TODO: Complete this function to update weights using the Q-Learning update rule
        if done:
            ...
            target_utility = self.__compute_utility_from_features(self.feature_extractor.extract_features(env, next_state))

        # Calculate the Q-value of the current state and action
        current_q_value = self.compute_q(env, state, action)

        # Compute the temporal difference
        temporal_difference = reward + self.discount_factor * target_utility - current_q_value

        # Update weights based on the temporal difference and features of the current state
        current_state_features = self.feature_extractor.extract_features(env, state)
        for feature, value in current_state_features.items():
            self.weights[action][feature] += self.learning_rate * temporal_difference * value
